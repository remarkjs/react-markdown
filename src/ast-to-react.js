...
// variable to cache the root node tree for walking
let root

function astToReact(node, options, parent = {}, index = 0) {
  const renderer = options.renderers[node.type];

  // the first node to come through astToReact() should be the tree root node. Cache this node for walking later
  if (node.type === 'root') {
    root = node;
  }
